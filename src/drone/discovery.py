...
def discover_ros_info(master_uri: str = "") -> Dict:
    ...
    uri = master_uri or os.environ.get("ROS_MASTER_URI",
                                        "http://192.168.11.1:11311")
    result: Dict = {"topics": [], "services": [], "clover_ns": None,
                    "ok": False, "error": "", "uri": uri}
    try:
        import xmlrpc.client
        code, msg, state = (xmlrpc.client.ServerProxy(uri)
                            .getSystemState("/discovery"))
        if code != 1:
        ...
        result["ok"]        = True
        log.debug(f"discovery: {len(result['topics'])} топиков, "
...
def format_ros_report(info: Dict) -> str:
    if not info["ok"]:
        return (f"Rosmaster ({info['uri']}) не ответил.\n"
                f"Ошибка: {info['error']}\n\n"
                "Проверьте:\n"
    ...
    clover = [s for s in info["services"]
              if any(k in s for k in
                     ("clover", "get_telemetry", "navigate", "land",
                      "set_velocity", "aruco"))]
    ...
    cam_topics = [t for t in info["topics"]
                  if any(k in t for k in ("camera", "image", "video"))]
    lines.append(f"\nТопики камеры ({len(cam_topics)}):")
    lines += [f"  {t}" for t in cam_topics[:10]]

    lines.append(f"\nВсего: {len(info['topics'])} топиков, "
                 f"{len(info['services'])} сервисов")
    return "\n".join(lines)
