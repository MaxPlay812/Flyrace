...
def _sshpass_ok() -> bool:
    try:
        subprocess.run(["sshpass", "--version"], capture_output=True, timeout=2)
        return True
...
def ssh_exec(
    ...
    if password and _sshpass_ok():
        argv = ["sshpass", "-p", password, "ssh"] + _SSH_OPTS + [f"{user}@{host}", cmd]
    else:
...
class DroneAutoSetup:
    ...
    def _run(self):
        ...
        for line in nraw.splitlines():
            if "NRestarts=" in line:
                try:
                    ...
                    n_restarts = 0  # unparseable counter → treat as no restarts
        is_active = active.strip() == "active"
        self._p(
            2, f"clover: {active.strip()}  (перезапусков: {n_restarts})", ok=is_active
        )
        ...
        all_svcs = [
            s.strip() for s in svc_raw.splitlines() if s.strip().startswith("/")
        ]
        key_svcs = [
            s
            for s in all_svcs
        ...
        if not key_svcs:
            ...
            self._p(
                5,
                f"Clover-сервисы не найдены "
                f"(всего {len(all_svcs)} сервисов в rosmaster)",
                ok=False,
            )
            self._on_done(
                False,
        ...
        if self.found_ns and self.found_ns != "/clover":
            ...
            self._p(
                5,
                f"Найдено {len(key_svcs)} сервисов, "
        ...
        telem_svc = next((s for s in key_svcs if s.endswith("/get_telemetry")), None)
        if telem_svc:
            _, traw = self._ssh(
                f'bash -lc "rosservice call {telem_svc} \\"frame_id: \'map\'\\" 2>&1"',
                timeout=8,
            )
